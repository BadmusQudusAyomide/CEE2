...
<form method="post" id="updateExamineeFrm">
     ...
     <legend>School</legend>
        <?php 
            $exmneSchool = $selExmne['exmne_School'];
            $selSchool = $conn->query("SELECT * FROM school_tbl WHERE cou_id='$exmneSchool' ")->fetch(PDO::FETCH_ASSOC);
         ?>
         <select class="form-control" name="exSchool">
           <option value="<?php echo $exmneSchool; ?>"><?php echo $selSchool['cou_name']; ?></option>
           <?php 
             $selSchool = $conn->query("SELECT * FROM school_tbl WHERE cou_id!='$exmneSchool' ");
             while ($selSchoolRow = $selSchool->fetch(PDO::FETCH_ASSOC)) { ?>
              <option value="<?php echo $selSchoolRow['cou_name']; ?>"><?php echo $selSchoolRow['cou_name']; ?></option>
            <?php  }
            ?>
         </select>
       </div>
